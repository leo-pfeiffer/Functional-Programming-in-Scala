...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy(x => (x)).map((k, v) => (k, v.length)).toList.sortWith(_._1 < _._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    // s.map(wordOccurrences).flatten.groupBy(_._1).map((k, v) => (k, v.map(_._2).sum)).toList
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    ...
    def subCombs(occurrence: (Char, Int)): Occurrences =
      occurrence match {
        case (char, count) =>
          ( 
            for n <- 1 to count
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    ...
    x.foldLeft[Occurrences](List())((ls, next) => {
      yMap.get(next._1) match {
        case Some(n: Int) => {
          if next._2 - n > 0 then (next._1, next._2 - n) :: ls
          else ls
        }
        case None => next :: ls
      }
    ...
    def iter(occurrences: Occurrences): List[Sentence] = {
      if occurrences.isEmpty then List(List())
        ...
        for {
          ...
          rest <- iter(subtract(occurrences, combination))
        } yield word :: rest
      }
    }
